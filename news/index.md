...
- Set num.threads=2 as default; respect environment variables and
  options
...
- Add time.interest option to restrict unique survival times (faster and
  saves memory)
...
- Faster permutation variable importance for high dimensional data
  (thanks to Roman Hornung)
...
- Add x/y interface
- Internal changes (seed differences possible, prediction incompatible
  with older versions)
...
- Remove GenABEL from suggested packages (removed from CRAN). GenABEL
...
- Add ‘order’ mode for unordered factors for GenABEL SNP data (binary
...
- Add standard error estimation with the infinitesimal jackknife (now
  the default)

## ranger 0.8.2

- Add bias-corrected impurity importance (actual impurity reduction,
  AIR)
...
- Better formula interface: Support interactions terms and faster
  computation
...
- Add splitting by maximally selected rank statistics for regression
  forests
...
- Add splitting by maximally selected rank statistics for survival
  forests
- Bug fixes
...
- New CRAN version. New CRAN versions will be 0.x.0, development
  versions 0.x.y

## ranger 0.3.9

- Reduce memory usage of savest forest objects (changed child.nodeIDs
  interface)
...
- Add predict.all option in predict() to get individual predictions for
  each tree for classification and regression
...
- Add memory save option to save memory for very large datasets (but
  slower)
...
- Prediction forests are now handled like regression forests: MSE used
  for prediction error and permutation importance
...
- Fixed a bug: prediction function is now working for probability
  prediction forests
- Slot “predictions” for probability forests now contains class
  probabilities
