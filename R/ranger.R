...
##' @param splitrule Splitting rule. For classification and probability estimation "gini", "extratrees" or "hellinger" with default "gini". For regression "variance", "extratrees", "maxstat" or "beta" with default "variance". For survival "logrank", "extratrees", "C", "maxstat", or "brier" with default "logrank". 
...
ranger <- function(formula = NULL, data = NULL, num.trees = 500, mtry = NULL,
  ...
  if (is.null(splitrule)) {
    ...
    if (treetype == 5) {
      ...
      stop("Error: Brier splitrule applicable to survival data only.")
    }
